...
- [x] [A.G-02] Add validation/default-resolution coverage for each graph attribute.
...
- [x] [C-04] Implement `auto_status=true` synthesized file behavior when handler does not write status.
...
- [x] [RG-04] Artifact directory and status file contract validations pass on integration smoke run.
...
- [x] [D-03] Route terminal errors immediately to fail routing (no retries). Verified by executor terminal-error handling (`_execute_node_handler` + `_should_retry` + `_select_route_edge`) and passing coverage in `uv run pytest -q tests/engine/test_retry_goal_gate.py -k non_retryable_exception_does_not_retry_and_routes_fail_immediately` (1 passed).
- [x] [D-04] Surface pipeline structural errors during validation whenever possible. Verified `validate_graph` surfaces structural diagnostics (`start_node`, `reachability`, `edge_source_exists`, `edge_target_exists`, `condition_syntax`) and `_start_pipeline` blocks execution on ERROR diagnostics; rechecked with `uv run pytest -q tests/dsl/test_validator.py tests/api/test_validation_diagnostics.py` (58 passed).
- [x] [11.9-01] Convert each DoD bullet in spec 11.9 into evaluator tests. Completed by adding explicit missing `context.*` empty-string assertions in `tests/engine/test_conditions.py::test_missing_prefixed_context_key_resolves_to_empty_string`; together with existing evaluator tests this now covers `=`, `!=`, `&&`, `outcome`, `preferred_label`, `context.*` (including missing-key empty string), and empty-condition true; rechecked with `uv run pytest -q tests/engine/test_conditions.py` (12 passed).
- [x] [11.6-01] Convert each DoD bullet in spec 11.6 into per-handler contract tests. Verified existing coverage for start/exit/codergen/wait.human/conditional/parallel/fan-in/tool/custom-handler contracts in `tests/handlers/test_handlers.py` (e.g., noop handlers, codergen artifact writes, interviewer preferred-label routing, parallel + fan-in semantics, tool execution, custom registration) with runtime integration behavior validated by `tests/engine/test_executor.py` and `tests/integration/test_parity_matrix.py`.
- [x] [11.3-02] Add deterministic edge-selection conformance tests. Verified existing deterministic conformance coverage in `tests/engine/test_routing.py` (step-specific tests for condition/preferred-label/suggested-id/weight/lexical ordering, plus fallback determinism); rechecked with `uv run pytest -q tests/engine/test_routing.py` (13 passed).
- [x] [10.6-01] Add routing tests for `outcome=success` and `outcome=fail`. Verified existing routing coverage in `tests/engine/test_routing.py::TestRouting::test_priority_condition_over_weight`, `tests/engine/test_retry_goal_gate.py::TestRetryAndGoalGate::test_fail_edge_after_retry_exhaustion`, `tests/engine/test_retry_goal_gate.py::TestRetryAndGoalGate::test_failure_routing_prefers_outcome_fail_edge_over_other_true_conditions`, and executor branch-routing tests (`tests/engine/test_executor.py::test_executor_resolves_start_and_branches`); rechecked targeted cases with `uv run pytest -q ...` (4 passed).
- [x] [10.6-02] Add routing test for conjunction with `context.tests_passed=true`. Verified by new routing-level coverage in `tests/engine/test_routing.py::test_conjunction_with_context_tests_passed_true_routes_matching_edge` (`uv run pytest -q tests/engine/test_routing.py -k conjunction_with_context_tests_passed_true`).
- [x] [10.6-03] Add routing tests for inequality and `preferred_label` matching. Completed with explicit routing-level coverage in `tests/engine/test_routing.py::test_inequality_condition_routes_and_preferred_label_applies_when_condition_fails` (`condition="context.loop_state!=exhausted"`) and `tests/engine/test_routing.py::test_preferred_label_condition_routes_matching_edge` (`condition="preferred_label=Fix"` and `condition="preferred_label=Approve"`), mirroring Section 10.6 examples.
- [x] [10.7-01] Document unsupported future operators as non-implemented features. Verified Section 10.7 explicitly documents `contains`, `matches`, `OR`, `NOT`, and numeric comparisons as future extensions, while `evaluate_condition` and condition tests enforce only current `=` / `!=` with `&&` semantics (`uv run pytest -q tests/engine/test_conditions.py`, 12 passed).
- [x] [9.6-04] Emit interview lifecycle events. Verified existing implementation: `WaitHumanHandler.execute` emits `InterviewStarted`, `InterviewCompleted`, and `InterviewTimeout` via runtime events (`attractor/handlers/builtin/wait_human.py`), with interview runtime-event coverage in `tests/engine/test_executor.py::test_executor_emits_parallel_and_interview_runtime_events` and timeout-path coverage in `tests/handlers/test_handlers.py` wait-human timeout cases.
- [x] [9.6-05] Emit checkpoint-saved events. Implemented spec-aligned emission by only publishing `CheckpointSaved(node_id=...)` when a checkpoint write actually persists, and added executor coverage for both paths (`test_executor_emits_typed_runtime_events_for_ui_consumers` with persisted `logs_root`, `test_executor_does_not_emit_checkpoint_event_when_nothing_is_persisted`) plus checkpoint-failure coverage (`test_finalize_persists_checkpoint_and_failure_event_when_runner_exception_becomes_fail_outcome`), rechecked with `uv run pytest -q tests/engine/test_executor.py::TestExecutor::test_executor_emits_typed_runtime_events_for_ui_consumers tests/engine/test_executor.py::TestExecutor::test_executor_does_not_emit_checkpoint_event_when_nothing_is_persisted tests/engine/test_checkpointing.py::TestCheckpointAndArtifacts::test_finalize_persists_checkpoint_and_failure_event_when_runner_exception_becomes_fail_outcome`.
- [x] [9.6-06] Support observer callback consumption and streaming consumption. Verified existing observer callback consumption via `PipelineExecutor(on_event=...)` + `_emit_event` dispatch (`attractor/engine/executor.py`) and streaming consumption via `GET /pipelines/{id}/events` SSE history/live stream (`attractor/api/server.py::pipeline_events`); rechecked with `uv run pytest -q tests/engine/test_executor.py::TestExecutor::test_executor_emits_typed_runtime_events_for_ui_consumers tests/api/test_pipeline_events_endpoint.py::test_pipeline_events_replays_history_without_replaying_same_event_from_live_queue tests/api/test_backend_invariance.py::test_pipeline_stream_includes_executor_typed_runtime_events` (3 passed).
- [x] [9.1-01] Implement transform interface (`apply(graph) -> graph`) and pipeline execution order. Verified existing implementation in `attractor/transforms/base.py` (`Transform.apply`) and `attractor/api/server.py::_build_transform_pipeline` (ordered built-ins then registered transforms), rechecked with `uv run pytest -q tests/api/test_validation_diagnostics.py::test_start_pipeline_runs_stylesheet_transform_before_validation tests/api/test_validation_diagnostics.py::test_preview_runs_custom_transforms_after_builtin_transforms` (2 passed).
- [x] [8.4-02] Apply provider keys for `llm_provider`. Verified existing implementation: stylesheet parsing accepts `llm_provider` (`attractor/transforms/stylesheet.py` `_ALLOWED_PROPERTIES` + `_apply_rules`) and provider propagation/precedence is asserted by transform coverage (`tests/transforms/test_transforms.py::test_stylesheet_specificity_and_explicit_override`, `tests/transforms/test_transforms.py::test_model_attr_precedence_node_then_stylesheet_then_graph_default`, `tests/transforms/test_transforms.py::test_stylesheet_spec_example_fixture_resolves_model_provider_and_reasoning`).
- [x] [8.3-02] Implement specificity ordering and tie-break by later rule of equal specificity. Re-verified against spec Section 8.3: `ModelStylesheetTransform` selects declarations via `(specificity, rule.order)` in `attractor/transforms/stylesheet.py`, with equal-specificity ordering and higher-specificity override coverage in `tests/transforms/test_transforms.py::test_stylesheet_multiple_matching_classes_use_rule_order_for_equal_specificity` and `::test_stylesheet_multiple_matching_classes_still_yield_to_id_specificity`.
- [x] [8.3-01] Implement selector matching for `*`, `.class`, and `#node_id`. Re-verified against spec Section 8.3: `ModelStylesheetTransform` matches universal/class/id selectors in `attractor/transforms/stylesheet.py::_selector_matches`, and transform coverage exercises all three selector kinds (`tests/transforms/test_transforms.py::test_stylesheet_specificity_and_explicit_override` plus class/id selector tests), so this was checklist state drift.
- [x] [8.1-01] Parse stylesheet from graph attribute and apply as defaults-only transform. Re-verified against spec 8.1/8.5: parser extracts `graph.model_stylesheet` and `ModelStylesheetTransform` applies it while preserving explicit node attrs; rechecked with `uv run pytest -q tests/dsl/test_parser.py::TestDotParserDefinitionOfDone11_1::test_extracts_goal_label_and_model_stylesheet_graph_attributes tests/dsl/test_parser.py::TestDotParserDefinitionOfDone11_1::test_node_class_attribute_merges_stylesheet_attributes tests/api/test_validation_diagnostics.py::test_start_pipeline_runs_stylesheet_transform_before_validation` (3 passed).
- [x] [7.2-03] Implement `reachability` rule. Verified existing behavior in `validate_graph` (`attractor/dsl/validator.py`) which emits `reachability` ERROR diagnostics for nodes outside the start-node traversal; rechecked via `uv run pytest -q tests/dsl/test_validator.py -k reachability` (1 passed).
- [x] [6.4-03] Implement `CallbackInterviewer` delegation path. Verified existing implementation against spec pseudocode (`CallbackInterviewer.ask` returns `callback(question)` in `attractor/interviewer/implementations.py`) and rechecked with `uv run pytest -q tests/interviewer/test_interviewer.py -k callback_interviewer` (1 passed).
- [x] [5.6-02] Ensure each stage directory includes `prompt.md`, `response.md`, `status.json`. Verified existing behavior: executor writes all three files in `attractor/engine/executor.py::_write_stage_artifacts`, with coverage in `tests/engine/test_checkpointing.py::test_artifacts_and_checkpoint_written_each_step` and `tests/engine/test_checkpointing.py::test_end_to_end_run_directory_structure_includes_terminal_stage_artifacts`.
- [x] [4.2-01] Implement registration API with replacement behavior for duplicate keys. Verified existing replacement behavior in `HandlerRegistry.register` (`handlers[type]=handler` overwrite semantics) aligns with Section 4.2; registry precedence coverage remains green in `uv run pytest -q tests/handlers/test_handlers.py -k registry_resolution`.
- [x] [2.6-02] Resolve `type` override before shape-based handler mapping. Verified explicit-`type` precedence in `HandlerRegistry.resolve_handler_type` (checks registered `type` before shape mapping) and rechecked with `uv run pytest -q tests/handlers/test_handlers.py -k "registry_resolution_by_shape_and_type or registry_resolution_precedence_levels"` (4 passed).
- [x] [2.8-02] Ensure explicit `type` attribute overrides shape mapping. Verified `HandlerRegistry.resolve_handler_type` checks registered explicit `type` before shape mapping, and `uv run pytest -q tests/handlers/test_handlers.py -k 'registry_resolution_by_shape_and_type or registry_resolution_precedence_levels'` passes.
- [x] [2.3-05] Accept optional statement semicolons. Verified parser semicolon tolerance in `attractor/dsl/parser.py` (`parse_graph` consumes optional/consecutive `SEMI` tokens around statements) and rechecked with `uv run pytest -q tests/dsl/test_parser.py -k 'semicolon or mixed_statement_ordering_without_semicolons'` (3 passed).
- [x] [2.4-02] Parse signed integers and floats. Re-verified against spec Section 2.4: tokenizer accepts optional `+/-` signed ints/floats in `attractor/dsl/parser.py`, and parser coverage confirms signed literal parsing (`tests/dsl/test_parser.py::test_lexes_all_supported_typed_values`, `tests/dsl/test_parser.py::test_strips_comments_before_parsing_signed_numeric_literals`).
- [x] [2.7-01] Implement edge `label` for preferred-label routing. Re-verified `select_next_edge` matches normalized `edge.attrs["label"]` against `outcome.preferred_label` in `attractor/engine/routing.py`, with passing focused coverage in `tests/engine/test_routing.py` (`preferred_label_then_suggested_ids`, `deterministic_step_2_preferred_label_match`) and edge-label parse coverage in `tests/dsl/test_parser.py` (`parses_all_supported_edge_attributes`).
- [x] [2.7-03] Implement `weight` for deterministic prioritization. Re-verified spec-aligned deterministic weight routing in `attractor/engine/routing.py::_best_by_weight_then_lexical` with focused coverage in `tests/engine/test_routing.py` (`test_weight_then_lexical_tiebreak_for_unconditional`, `test_deterministic_step_4_highest_weight_unconditional`, `test_deterministic_step_5_lexical_tiebreak`) via `uv run pytest -q ...` (3 passed).
- [x] [2.9-01] Expand chained declarations into pairwise edges. Verified existing parser chain expansion loop in `attractor/dsl/parser.py::parse_node_or_edge` and explicit coverage in `tests/dsl/test_parser.py::test_expands_chained_edges_into_pairwise_edges`.
- [x] [2.10-01] Flatten subgraph wrappers while preserving contained nodes/edges. Verified existing flattening in `attractor/dsl/parser.py::parse_statement` (subgraph statements parse into the same top-level `DotGraph`) with explicit node+edge retention coverage in `tests/dsl/test_parser.py::test_subgraph_contents_are_flattened_into_top_level_graph`.
- [x] [2.10-02] Apply subgraph-local defaults to enclosed nodes unless overridden. Re-verified against spec Section 2.10: parser subgraph scoping applies `node [...]` defaults through child-scope inheritance (`attractor/dsl/parser.py`), and focused coverage passed in `uv run pytest -q tests/dsl/test_parser.py -k subgraph_scope_defaults` (1 passed).
- [x] [3.3-02] Implement normalized preferred-label matching (case/trim/accelerator stripping). Verified `select_next_edge` normalizes `outcome.preferred_label` via trim + `_normalize_label` (casefold + accelerator stripping) in `attractor/engine/routing.py`, with routing coverage in `tests/engine/test_routing.py::test_preferred_label_then_suggested_ids` and `::test_deterministic_step_2_preferred_label_match`.
- [x] [3.3-04] Implement weight-descending selection for unconditional edges. Verified existing Section 3.3 Step 4/5 behavior in `attractor/engine/routing.py::_best_by_weight_then_lexical` (`weight` DESC then lexical target ASC), rechecked with `uv run pytest -q tests/engine/test_routing.py -k 'deterministic_step_4_highest_weight_unconditional or weight_then_lexical_tiebreak_for_unconditional'` (2 passed).
- [x] [3.7-01] Route fail outcomes to fail-edge (`condition="outcome=fail"`) when present. Verified existing fail-edge prioritization in `attractor/engine/executor.py::_select_route_edge`; rechecked with `uv run pytest -q tests/engine/test_retry_goal_gate.py -k failure_routing_prefers_outcome_fail_edge_over_other_true_conditions` (1 passed).
- [x] [4.8-04] Serialize branch results into `parallel.results`. Re-verified `ParallelHandler.execute` returns `context_updates["parallel.results"]` with serialized per-branch payloads (`attractor/handlers/builtin/parallel.py`), and focused coverage passes in `uv run pytest -q tests/handlers/test_handlers.py::TestBuiltInHandlers::test_parallel_branches_keep_context_updates_isolated_per_branch tests/handlers/test_handlers.py::TestBuiltInHandlers::test_parallel_handler_supports_error_policies tests/integration/test_parity_matrix.py::TestParityMatrixSubset::test_reference_workflow_includes_llm_conditions_human_and_parallel` (5 passed).
- [x] [4.9-01] Read `parallel.results` and fail when empty. Verified existing in `FanInHandler.execute` (`raw_results = runtime.context.get("parallel.results", [])`, then normalized-empty guard returns `FAIL` with `"No parallel results to evaluate"`), and rechecked fan-in behavior with `uv run pytest -q tests/handlers/test_handlers.py -k fan_in` (3 passed).
- [x] [4.9-04] Publish selected candidate via `parallel.fan_in.*` context keys. Verified `FanInHandler.execute` publishes `parallel.fan_in.best_id` + `parallel.fan_in.best_outcome` and `tests/handlers/test_handlers.py` asserts both in fan-in backend and heuristic paths (`uv run pytest -q tests/handlers/test_handlers.py -k fan_in`, 3 passed).
- [x] [4.12-01] Document and support custom handler registration by type string. Verified spec 4.12 registration flow is implemented via `HandlerRegistry.register` + `build_default_registry(..., extra_handlers=...)` and exercised by `tests/handlers/test_handlers.py` (`test_registry_allows_plugin_injection_via_builder`, `test_registry_resolution_by_shape_and_type`) with focused recheck (`uv run pytest -q ...`, 3 passed).
- [x] [5.4-02] Implement fidelity precedence: edge -> target node -> graph default -> `compact`. Verified existing precedence in `PipelineExecutor._resolve_runtime_fidelity` (including `compact` fallback) and rechecked with `uv run pytest -q tests/engine/test_executor.py -k "resolves_runtime_fidelity_from_graph_default or node_fidelity_overrides_graph_default or edge_fidelity_overrides_node_and_graph_defaults"` (3 passed) plus full suite `uv run pytest -q` (495 passed).
- [x] [7.1-02] Block execution when any ERROR diagnostic exists. Verified existing behavior against spec 7.1: `_start_pipeline` returns `status=\"validation_error\"` when any error-severity diagnostic is present (`attractor/api/server.py`), rechecked with `uv run pytest -q tests/api/test_validation_diagnostics.py -k start_pipeline_validation_error_payload_shape` (1 passed).
- [x] [7.2-06] Implement `exit_no_outgoing` rule. Verified existing spec-aligned behavior: `validate_graph` emits ERROR `exit_no_outgoing` for terminal nodes with outgoing edges (`attractor/dsl/validator.py`), matching Section 7.2 in `attractor-spec.md`; rechecked with `uv run pytest -q tests/dsl/test_validator.py -k edge_target_exists_and_start_incoming_exit_outgoing` (1 passed).
- [x] [7.2-10] Implement `fidelity_valid` warning rule. Verified existing implementation in `attractor/dsl/validator.py::_validate_fidelity_values` (graph/node/edge fidelity diagnostics) and rechecked coverage with `uv run pytest -q tests/dsl/test_validator.py -k retry_target_and_fidelity_warnings` (1 passed).
- [x] [7.2-11] Implement `retry_target_exists` warning rule. Completed by normalizing retry targets with `.strip()` in `attractor/dsl/validator.py::_validate_retry_targets` so validator existence checks match runtime retry-target resolution; covered by `tests/dsl/test_validator.py::test_retry_target_exists_ignores_surrounding_whitespace_for_valid_targets` (`uv run pytest -q ...`, 2 passed with companion retry warning test).
- [x] [B-01] Add one conformance test per shape-to-handler mapping row. Verified `tests/handlers/test_handlers.py::test_registry_shape_mapping_covers_all_spec_shapes` covers each Section 2.8 mapping row, rechecked with `uv run pytest -q tests/handlers/test_handlers.py -k test_registry_shape_mapping_covers_all_spec_shapes` (9 passed).
- [x] [10.1-01] Keep expression language minimal and deterministic for routing. Verified `evaluate_condition` remains constrained to `=`/`!=` clauses with `&&` splitting (`attractor/engine/conditions.py`), routing remains deterministic via weight-desc/lexical tie-break (`attractor/engine/routing.py`), and focused coverage passes (`uv run pytest -q tests/engine/test_conditions.py tests/engine/test_routing.py`, 28 passed).
- [x] [10.3-01] Evaluate clauses left-to-right with logical AND semantics. Verified existing: `evaluate_condition` iterates `&&` clauses in source order and returns `False` immediately on first failed clause (`attractor/engine/conditions.py`), consistent with spec Sections 10.3/10.5; rechecked with `uv run pytest -q tests/engine/test_conditions.py -k equals_not_equals_and` (1 passed).
- [x] [10.3-02] Treat missing context keys as empty string. Verified `evaluate_condition` routes unresolved `context.*` keys through `Context.get_context_path` which returns `""` on missing segments, matching spec 10.3; rechecked with `uv run pytest -q tests/engine/test_conditions.py -k missing_prefixed_context_key_resolves_to_empty_string` (1 passed).
- [x] [10.5-01] Return true for empty condition. Re-verified spec Section 10.5 behavior: `evaluate_condition` returns true when condition is empty (`attractor/engine/conditions.py`), covered by `tests/engine/test_conditions.py::test_empty_condition_true` (`uv run pytest -q tests/engine/test_conditions.py -k empty_condition_true`, 1 passed).
- [x] [10.5-02] Evaluate `!=` and `=` clauses correctly. Re-verified Section 10.5 evaluator semantics in `attractor/engine/conditions.py` (`=` requires exact equality, `!=` requires exact inequality) and confirmed coverage in `tests/engine/test_conditions.py` (`test_equals_not_equals_and`, `test_typed_literals_with_equals_and_not_equals`, `test_string_comparison_is_exact_and_case_sensitive`) with `uv run pytest -q tests/engine/test_conditions.py` (12 passed).
- [x] [RG-01] Every subsection above has at least one completed implementation task. Reassessed by checklist coverage audit: every subsection has at least one completed item, including previously deferred subsection IDs now completed (`8.3-*`, `10.6-*`, `11.6-*`, `11.9-*`), so this release-gate criterion is now satisfied.
- [x] [A.E-03] Add runtime tests for `fidelity`, `thread_id`, and `loop_restart` behavior. Completed by adding explicit edge-thread precedence coverage in `tests/engine/test_executor.py::test_executor_uses_edge_thread_id_when_full_fidelity_node_has_no_thread_id`, alongside existing fidelity precedence tests and loop-restart relaunch/reset tests in the same module.
- [x] [A.E-02] Add routing tests for `condition`, `weight`, and `label` interplay. Verified existing interplay coverage in `tests/engine/test_routing.py` (`test_deterministic_step_1_condition_match_wins_over_other_signals`, `test_deterministic_step_2_preferred_label_match`, `test_priority_condition_over_weight`, `test_weight_then_lexical_tiebreak_for_unconditional`); rechecked with `uv run pytest -q tests/engine/test_routing.py` (16 passed).
