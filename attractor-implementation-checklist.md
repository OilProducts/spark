...
- [ ] [A.E-02] Add routing tests for `condition`, `weight`, and `label` interplay. Deferred because a fresh scan shows this interplay is already covered by routing tests for condition precedence, preferred-label selection, and weight/lexical tie-break determinism in `tests/engine/test_routing.py`, so this remains checklist state drift.
