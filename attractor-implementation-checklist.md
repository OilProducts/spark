...
- [ ] [8.2-03] Enforce class name format and declaration syntax validation. Deferred because stylesheet linting already enforces selector class-name format (`_STYLESHEET_CLASS_RE`) and declaration syntax/semicolon/colon structure in `attractor/dsl/validator.py::_lint_stylesheet_syntax`, with coverage in `tests/dsl/test_validator.py` stylesheet syntax tests (including invalid class selectors and malformed declarations), so this is checklist state drift.
