...
import { stages, labels, type Mission, type Fields, type Stage } from './MissionsPanel'
...
type Core = 'title' | 'description' | 'stage' | 'archived'
const fieldLabels: Record<Core, string> = { title: 'Title', description: 'Description', stage: 'Stage', archived: 'Archived' }
function display(key: Core, value: Fields[Core]) {
    ...
    return <section aria-label="Mission details" className={`flex min-h-0 min-w-0 flex-col rounded-md border border-border bg-card ${narrow ? 'w-full' : 'w-[28rem] shrink-0'}`} onKeyDown={e => {
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4 space-y-4 text-sm">
            ...
            {changed && <div role="status" className="space-y-3 rounded-md border border-border bg-muted/20 p-3">
                ...
                <dl className="space-y-2">{(Object.keys(fieldLabels) as Core[]).filter(key => latest.fields[key] !== editing?.fields[key]).map(key => <div key={key}><dt className="font-medium">{fieldLabels[key]}</dt><dd className="whitespace-pre-wrap break-words text-muted-foreground">{display(key, latest.fields[key])}</dd></div>)}</dl>
            ...
            {editing && <details className="border-t border-border pt-4"><summary className="cursor-pointer rounded-sm font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Activity</summary><ol className="mt-2 space-y-3">{(latest ?? editing).activity.map(a => <li key={a.revision} className="border-b border-border pb-3">
                ...
                {a.after && <ul className="mt-1 space-y-1">{(Object.keys(fieldLabels) as Core[]).filter(key => a.after?.[key] !== a.before?.[key]).map(key => <li key={key} className="whitespace-pre-wrap break-words">{fieldLabels[key]}: {a.before && `${display(key, a.before[key])} → `}{display(key, a.after![key])}</li>)}</ul>}
