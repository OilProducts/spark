...
it('validates fields, saves explicitly, and retains conflicts until Discard', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Save connection settings' })).toBeDisabled()
    await user.clear(target)
    await user.type(target, 'https://example.com')
    expect(saveConnectionSettings).not.toHaveBeenCalled()
    await user.click(screen.getByRole('button', { name: 'Save connection settings' }))
    ...
    await user.click(screen.getByRole('button', { name: 'Discard connection changes' }))
