...
export function ClientPreferencesEditor() {
    ...
    return <Card>
        <CardHeader><CardTitle>Client preferences</CardTitle></CardHeader>
        <CardContent className="space-y-3">
            <p className="text-xs">Sidebar width applies after saving. Editor mode and graph choices are defaults for newly opened flows.</p>
            <fieldset disabled={!editor.draft || editor.pending} className="space-y-3">
                ...
                <Field><FieldLabel htmlFor="preference-sidebar-width">Editor sidebar width (pixels)</FieldLabel>
                    <Input id="preference-sidebar-width" type="number" min={256} max={560} step={1}
                        aria-invalid={editor.invalidWidth} aria-describedby="preference-sidebar-help"
                    ...
                    {editor.invalidWidth && <p role="alert">Choose a whole number from 256 to 560.</p>}
                </Field>
                <Field><FieldLabel htmlFor="preference-home-split">Home sidebar primary split (0–1)</FieldLabel>
                    <Input id="preference-home-split" type="number" min={0} max={1} step={0.01}
                        aria-invalid={editor.invalidSplit} aria-describedby="preference-home-split-help"
                    ...
                    <p id="preference-home-split-help" className="text-xs">Leave blank for automatic sizing. Both panes retain their minimum height.</p>
                    {editor.invalidSplit && <p role="alert">Choose a number from 0 to 1.</p>}
                </Field>
                {([
                ...
                {(Object.keys(runPresentationChoices) as (keyof typeof runPresentationChoices)[]).map((key) => <Field key={key}>
                    <FieldLabel htmlFor={`run-preference-${key}`}>Run {key.replaceAll('_', ' ')}</FieldLabel>
                    <NativeSelect id={`run-preference-${key}`} value={editor.draft?.run_presentation?.[key] ?? ''} onChange={(event) => editor.setDraft((draft) => draft && ({ ...draft, run_presentation: { ...draft.run_presentation, [key]: event.target.value || null } as RunPresentation }))}>
                        <option value="">Default</option>{runPresentationChoices[key].map((value) => <option key={value} value={value}>{value}</option>)}
                    </NativeSelect>
                </Field>)}
                <Field><FieldLabel htmlFor="run-preference-graph-height">Run graph height</FieldLabel>
                    <Input id="run-preference-graph-height" aria-invalid={editor.invalidGraphHeight} type="number" min={280} max={960} value={editor.draft?.run_presentation?.graph_height ?? ''} onChange={(event) => editor.setDraft((draft) => draft && ({ ...draft, run_presentation: { ...draft.run_presentation, graph_height: event.target.value === '' ? null : Number(event.target.value) } }))} />
                    {editor.invalidGraphHeight && <p role="alert">Choose a whole number from 280 to 960.</p>}
                </Field>
            </fieldset>
