...
export function LlmProfilesEditor() {
    ...
    return <Card><CardHeader><CardTitle>LLM profiles</CardTitle></CardHeader><CardContent className="space-y-3">
        ...
        <fieldset disabled={editor.pending || !editor.saved} className="space-y-4">
            {profiles.map((profile, index) => <fieldset key={index} className="space-y-2 rounded border p-3">
                <legend>LLM profile {index + 1}</legend>
                <label className="block">Profile ID<Input aria-label={`LLM profile ${index + 1} ID`} value={profile.id} onChange={(event) => change(index, { id: event.target.value })} /></label>
                ...
                <label className="block">Endpoint<Input aria-invalid={!validEndpoint(profile.base_url)} value={profile.base_url} onChange={(event) => change(index, { base_url: event.target.value })} /></label>
                {!validEndpoint(profile.base_url) && <p role="alert">Use an HTTP(S) endpoint without embedded credentials, query parameters or fragments.</p>}
                <label className="block">Credential environment variable<Input aria-invalid={!!profile.api_key_env && !/^[A-Za-z_][A-Za-z0-9_]*$/.test(profile.api_key_env)} value={profile.api_key_env ?? ''} onChange={(event) => change(index, { api_key_env: event.target.value || undefined })} /></label>
                <p className="text-xs">Saved credential status: {editor.saved?.credential_status?.[profile.id] ?? 'unsaved'}</p>
                <label className="block">Models (one per line)<textarea className="block w-full rounded border p-2" value={profile.models.join('\n')} onChange={(event) => change(index, { models: event.target.value.split('\n') })} /></label>
                <label className="block">Default model<NativeSelect aria-label="Default model" value={profile.default_model ?? ''} onChange={(event) => change(index, { default_model: event.target.value || undefined })}>
...
function ExecutionProfileFields({ profile, index, change, remove, onValidity }: {
    profile: ExecutionProfileSettings; index: number; change: (patch: Partial<ExecutionProfileSettings>) => void; remove: () => void; onValidity: (invalid: boolean) => void
}) {
    ...
    return <fieldset className="space-y-2 rounded border p-3"><legend>Execution profile {index + 1}</legend>
        <label className="block">Profile ID<Input value={profile.id} onChange={(event) => change({ id: event.target.value })} /></label>
        <label className="block">Label<Input value={profile.label} onChange={(event) => change({ label: event.target.value })} /></label>
        ...
        <label className="block">Container image<Input aria-invalid={profile.mode === 'local_container' && !profile.image?.trim()} value={profile.image ?? ''} onChange={(event) => change({ image: event.target.value || undefined })} /></label>
        {profile.mode === 'local_container' && !profile.image?.trim() && <p role="alert">A container image is required.</p>}
        ...
        <label className="block">Metadata (JSON object)<textarea className="block w-full rounded border p-2 font-mono" aria-invalid={!!error} value={metadata} onChange={(event) => {
        ...
        <Button variant="outline" onClick={remove}>Delete execution profile {profile.id}</Button>
    </fieldset>
}
...
export function ExecutionProfilesEditor() {
    ...
    return <Card><CardHeader><CardTitle>Execution profiles</CardTitle></CardHeader><CardContent className="space-y-3">
        ...
        <fieldset disabled={editor.pending || !editor.saved} className="space-y-3">
            ...
            {profiles.map((profile, index) => <ExecutionProfileFields key={`${editor.saved?.revision}-${generation}-${index}`} profile={profile} index={index} change={(patch) => change(index, patch)}
