...
test('run summary panel renders populated metadata for items 9.1-01 and 9.6-02', async ({ page }) => {
  ...
  await expect(page.getByTestId('run-summary-working-directory-note')).toContainText(`${projectPath}/workspace`)
  await expect(page.getByTestId('run-summary-project-path')).toContainText(projectPath)
  await expect(page.getByTestId('run-summary-git-ref')).toContainText('feature/traceability @ fedcba9')
