...
test('runtime settings Save, Discard, validation, navigation guard, live conflict, and reload', async ({ page, context }) => {
    ...
    try {
        ...
        await expect(card(page, 'Runtime paths').getByRole('button', { name: 'Save', exact: true })).toBeDisabled()
        await card(page, 'Runtime paths').getByRole('button', { name: 'Discard', exact: true }).click()
        ...
        await card(second, 'Runtime paths').getByRole('button', { name: 'Save', exact: true }).click()
        ...
        await expect(input).toHaveValue('/tmp/spark-settings-first')
        await card(page, 'Runtime paths').getByRole('button', { name: 'Save', exact: true }).click()
...
test('workspace model defaults require Save and survive reload; dirty drafts survive other clients', async ({ page, context }) => {
    ...
    try {
        ...
        await card(page, 'Model defaults (Workspace)').getByRole('button', { name: 'Save', exact: true }).click()
        ...
        await card(other, 'Model defaults (Workspace)').getByRole('button', { name: 'Save', exact: true }).click()
        ...
        await expect(page.getByText('Settings changed since this document was read. Reload before saving.', { exact: false })).toBeVisible()
        await expect(page.getByLabel('Custom model', { exact: true })).toHaveValue('retained-draft')
        await card(page, 'Model defaults (Workspace)').getByRole('button', { name: 'Discard', exact: true }).click()
...
test('client preferences save explicitly, survive reload, and stay isolated between browser clients', async ({ page, browser }) => {
    ...
    try {
        ...
        await card(page, 'Client preferences').getByRole('button', { name: 'Save', exact: true }).click()
        ...
        await card(other, 'Client preferences').getByRole('button', { name: 'Save', exact: true }).click()
...
test('profile CRUD saves explicitly, retains conflicts, validates mounts, and survives reload', async ({ page }) => {
    ...
    try {
        ...
        await card(page, 'LLM profiles').getByRole('button', { name: 'Save', exact: true }).click()
        ...
        await expect(page.getByText('Settings changed since this document was read. Reload before saving.', { exact: false })).toBeVisible()
        await card(page, 'LLM profiles').getByRole('button', { name: 'Discard', exact: true }).click()
        ...
        await expect(card(page, 'Execution profiles').getByRole('button', { name: 'Save', exact: true })).toBeDisabled()
        ...
        await card(page, 'Execution profiles').getByRole('button', { name: 'Save', exact: true }).click()
...
test('connection settings validate, save, preserve running binding, and reload', async ({ page }) => {
    ...
    try {
        ...
        await expect(card(page, 'Server and client connections').getByRole('button', { name: 'Save', exact: true })).toBeDisabled()
        await port.fill('8123')
        await target.fill('https://spark.example')
        expect((await read()).connections.stored).toEqual(original.stored)
        await card(page, 'Server and client connections').getByRole('button', { name: 'Save', exact: true }).click()
        ...
        await card(page, 'Server and client connections').getByRole('button', { name: 'Discard', exact: true }).click()
        await expect(target).toHaveValue('https://spark.example')
    } finally {
        const current = await read()
...
test('provider and agent sections validate, save, report references and restart requirements, and survive reload', async ({ page }) => {
    ...
    try {
        ...
        await expect(card(page, 'Provider connections').getByRole('button', { name: 'Save', exact: true })).toBeDisabled()
        ...
        await card(page, 'Provider connections').getByRole('button', { name: 'Save', exact: true }).click()
        ...
        await card(page, 'Agent session limits').getByRole('button', { name: 'Save', exact: true }).click()
...
for (const width of [1440, 390]) {
    test(`Settings categories retain drafts and support keyboard access without overflow at ${width}px`, async ({ page }, testInfo) => {
        ...
        await card(page, 'Client preferences').getByRole('button', { name: 'Discard', exact: true }).click()
