...
#[test]
fn conversation_repository_batch_persistence_rebases_stale_snapshot_revisions() {
    let temp = tempfile::tempdir().expect("tempdir");
    let home = temp.path().join("spark-home");
    let project_path = "/projects/conversation-batch";
    ...
    let initial = json!({
        "schema_version": CONVERSATION_STATE_SCHEMA_VERSION,
        "revision": 5,
        "conversation_id": "conversation-batch",
        "conversation_handle": "amber-anchor",
        "project_path": project_path,
        "chat_mode": "chat",
        "provider": "codex",
    ...
    repo.write_snapshot(&initial).expect("initial snapshot");

    let mut stale = initial.clone();
    stale["revision"] = json!(2);
    stale["updated_at"] = json!("2026-01-01T00:00:06Z");
    let mut events = vec![
        json!({
            "type": "turn_upsert",
            "revision": 2,
    ...
    repo.persist_snapshot_with_events(&mut stale, &mut events)
        .expect("batch persist");

    assert_eq!(events[0]["revision"], 6);
    assert_eq!(events[1]["revision"], 7);
    assert_eq!(events[1]["state"]["revision"], 7);
    assert_eq!(stale["revision"], 7);
    let persisted = repo
        .read_snapshot("conversation-batch", Some(project_path))
        .expect("read")
        .expect("snapshot");
    ...
    assert!(persisted["turns"]
        ...
        .expect("turns")
        .iter()
        .any(|turn| turn["id"] == "turn-a"));
    let replay = repo
        .read_conversation_events_after("conversation-batch", project_path, 5)
        .expect("events");
    assert_eq!(
        replay
            .iter()
