...
use attractor_core::{DotGraph, LaunchContext, RunRecord};
...
fn gate_graph() -> DotGraph {
    parse_dot(
        r#"
        digraph Gate {
          start [shape=Mdiamond]
          review [shape=hexagon, prompt="Approve the change?"]
          approved [shape=box, prompt="ship"]
          rejected [shape=box, prompt="revise"]
          done [shape=Msquare]
          start -> review
...
fn record(run_id: &str, project_path: &Path) -> RunRecord {
    ...
    record.flow_name = "gate-contract.dot".to_string();
...
fn blocking_gate_waits_for_journaled_answer_and_routes_it() {
    ...
    assert_eq!(pending.payload["flow_name"], "gate-contract.dot");
    ...
    harness
        ...
        .append_event(
            ...
            human_gate_answered_event(
                ...
                Some("gate-contract.dot".to_string()),
