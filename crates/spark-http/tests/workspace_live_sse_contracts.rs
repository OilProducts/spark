...
async fn live_route_streams_run_segment_upserts_through_the_publisher() {
    ...
    // A failed run with a stored graph snapshot, checkpoint, and journaled
    ...
    record.flow_name = "segment-live.dot".to_string();
    record.status = "failed".to_string();
    let flow = "digraph SegLive { start [shape=Mdiamond] done [shape=Msquare] start -> done }";
    ...
    let paths = store
        .create_run(CreateRunRequest {
            ...
            graph_source: Some(flow.to_string()),
            graph_dot: Some(flow.to_string()),
            ..CreateRunRequest::default()
        })
        .expect("seed run");
    store
