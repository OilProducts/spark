...
### 1.3 Non-Goals
- [x] [1.3-01] Define the UI/runtime semantic boundary in `specs/ui-spec.md` so UI work does not alter engine semantics.
...
- [x] [2-01] Implement spec-first behavior mapping for UI controls and tests with direct references back to `specs/ui-spec.md` and `specs/attractor-spec.md`.
...
- [x] [9.3-03] Add copy/export actions for context inspection.
...
- [x] [9.4-02] Add filters by event type, node/stage, and severity/category.
...
- [x] [10.2-04] Add contract tests for each supported question type.
...
- [x] [10.3-02] Display timeout/default-applied/skipped outcome provenance in run timeline.
  - Evidence (2026-03-04): Evaluator `pass`; `RunsPanel` now renders provenance-aware interview timeline summaries (`accepted`, `skipped`, `timeout_default_applied`, `timeout_no_default`) sourced from runtime event payloads, with frontend contract `CID:10.3.02` and backend handler coverage in `tests/handlers/test_wait_human_handler.py`, plus screenshot-based visual QA review of run timeline surfaces in `frontend/artifacts/ui-smoke/08i-runs-panel-event-timeline.png`, `frontend/artifacts/ui-smoke/08j-runs-panel-event-timeline-filters.png`, and `frontend/artifacts/ui-smoke/08k-runs-panel-event-timeline-grouping-correlation.png`.
- [x] [10.3-03] Add tests for timeout fallback and explicit answer branches.
...
- [x] [10.4-03] Preserve order and auditability of grouped interactions.
  - Evidence (2026-03-04): Evaluator `pass`; `RunsPanel` now preserves grouped interaction order by timeline sequence (group order + in-group order) and renders per-item audit metadata (`Order #`, `Question ID`, `Received`), covered by frontend contract `CID:10.4.03` (`frontend/src/components/__tests__/ContractBehavior.test.tsx`, `tests/contracts/frontend/test_human_gate_discoverability_contracts.py`) with screenshot-based visual QA references `frontend/artifacts/ui-smoke/10b-human-gate-discoverability-runs.png`, `frontend/artifacts/ui-smoke/10c-human-gate-yes-no-confirmation-semantics.png`, and `frontend/artifacts/ui-smoke/10d-human-gate-freeform-interaction.png`.
...
- [x] [11.2-02] Add open/edit/save/re-open tests for advanced attribute coverage.
  - Evidence (2026-03-04): Evaluator `pass`; added open/edit/save/re-open API contract `tests/api/test_flow_save_validation.py::test_save_flow_open_edit_save_reopen_preserves_advanced_attrs_item_11_2_02` plus canonical serializer fix preserving explicit `false` advanced booleans in `frontend/src/lib/canonicalFlowModel.ts`, validated by `just test` and `uv run pytest -q`.
- [x] [11.2-03] Add regression tests for previously lossy edit paths.
...
- [x] [11.3-03] Add conflict handling when raw edit invalidates structured assumptions.
...
- [x] [11.4-03] Add tests proving extension attribute stability across edits.
...
- [x] [12.1-03] Add endpoint-level integration tests for happy path and common error cases.
  - Evidence (2026-03-04): Evaluator `pass`; added frontend contract `CID:12.1.03` covering endpoint-adapter happy paths plus common HTTP/schema error cases (including plain-text HTTP error detail fallback) in `frontend/src/components/__tests__/ContractBehavior.test.tsx`, bridged by `tests/contracts/frontend/test_api_integration_contracts.py`, with adapter error-detail handling updated in `frontend/src/lib/apiClient.ts` and validated by `just test` (`594 passed`) plus screenshot-based visual QA references `frontend/artifacts/ui-smoke/08-runs-panel.png`, `frontend/artifacts/ui-smoke/08b-runs-panel-populated-summary.png`, `frontend/artifacts/ui-smoke/10b-human-gate-discoverability-runs.png`, and `frontend/artifacts/ui-smoke/20a-plan-failure-rerun-enabled.png`.
...
- [x] [12.2-03] Ensure save paths remain non-destructive during API contract drift.
  - Evidence (2026-03-04): Evaluator `pass`; added frontend contract `CID:12.2.03` covering save-response shape drift (`{ saved: true }`) to verify raw→structured handoff remains blocked/non-destructive (raw draft preserved, no preview hydration after failed save) in `frontend/src/components/__tests__/ContractBehavior.test.tsx`, bridged by `tests/contracts/frontend/test_api_integration_contracts.py`, and tightened runtime handoff error sourcing to use latest store save error state in `frontend/src/components/Editor.tsx`; validated by `just test` (`597 passed`) with screenshot-based visual QA references `frontend/artifacts/ui-smoke/19-semantic-equivalence-noop-save.png`, `frontend/artifacts/ui-smoke/19a-semantic-equivalence-mismatch-blocked.png`, `frontend/artifacts/ui-smoke/19b-semantic-equivalence-round-trip-saved.png`, and `frontend/artifacts/ui-smoke/01-editor-shell.png` (`just ui-smoke` produced unrelated backend `ECONNREFUSED`/HTTP 500 failures in other scenarios).
...
- [x] [13.1-03] Verify diagnostic/status color contrast against accessibility standards.
  - Evidence (2026-03-05): Evaluator `pass`; added frontend contract `CID:13.1.03` with WCAG contrast-ratio checks across error/warning/info/success diagnostic-status palettes in `frontend/src/components/__tests__/ContractBehavior.test.tsx` (bridged by `tests/contracts/frontend/test_accessibility_contracts.py`), then strengthened runtime palettes in `frontend/src/index.css` and `frontend/src/components/RunsPanel.tsx`, with screenshot-based visual QA on `frontend/artifacts/ui-smoke/13-validation-panel-filter-sort.png`, `frontend/artifacts/ui-smoke/14-inline-diagnostic-badges.png`, `frontend/artifacts/ui-smoke/15-inspector-field-level-diagnostics.png`, `frontend/artifacts/ui-smoke/16-warning-only-execute-banner.png`, `frontend/artifacts/ui-smoke/17-diagnostic-transition-execute-state.png`, `frontend/artifacts/ui-smoke/18-diagnostic-navigation-to-canvas.png`, `frontend/artifacts/ui-smoke/08-runs-panel.png`, `frontend/artifacts/ui-smoke/08p-runs-panel-run-history-traceability.png`, and `frontend/artifacts/ui-smoke/08q-runs-panel-run-history-spec-plan-links.png` (`just ui-smoke` still reported unrelated failures in other scenarios).
...
- [x] [13.2-02] Ensure mobile and narrow viewport usability for core project and operational tasks.
  - Evidence (2026-03-05): Evaluator `pass`; narrow/mobile usability now includes stacked top-nav + mode tabs and wrapping action controls in `frontend/src/components/Navbar.tsx`, expanded frontend contract coverage in `frontend/src/components/__tests__/ContractBehavior.test.tsx` (`CID:13.2.02`), and dedicated 390x844 ui-smoke visual QA captures at `frontend/artifacts/ui-smoke/13a-mobile-projects-operations.png` and `frontend/artifacts/ui-smoke/13b-mobile-execution-controls.png` from `frontend/e2e/smoke/responsive-mobile.spec.ts`.
- [x] [13.2-03] Add viewport-based regression tests/screenshots.
  - Evidence (2026-03-05): Evaluator `pass`; added frontend contract `CID:13.2.03` to assert desktop (`1280`) vs narrow (`760`) responsive layout expectations for navbar, project operations, and execution controls in `frontend/src/components/__tests__/ContractBehavior.test.tsx`, bridged by `tests/contracts/frontend/test_accessibility_contracts.py`, plus desktop viewport regression smoke coverage in `frontend/e2e/smoke/responsive-mobile.spec.ts` with screenshot-based visual QA references `frontend/artifacts/ui-smoke/13a-mobile-projects-operations.png`, `frontend/artifacts/ui-smoke/13b-mobile-execution-controls.png`, `frontend/artifacts/ui-smoke/13c-desktop-projects-operations.png`, and `frontend/artifacts/ui-smoke/13d-desktop-execution-controls.png` (`just ui-smoke` reported unrelated backend `ECONNREFUSED`/HTTP 500 failures in other scenarios).
