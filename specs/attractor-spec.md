...
4. **Initialize:** Create the run directory, initial context, and checkpoint. Mirror graph attributes into the context, then merge any validated host-supplied launch context.
...
### 3.1.1 Host Launch Context

An implementation may provide optional host-supplied launch context when starting a run:

```
launch_context : Map<String, Any> -- optional semantic state injected before the first node runs
```

Rules:

- `launch_context` is optional. Missing `launch_context` is equivalent to an empty map.
- Every key in `launch_context` must be in the `context.*` namespace.
- Values must be serializable so they can be checkpointed and inspected later.
- Invalid keys are a run-start validation error.
- `launch_context` cannot populate `graph.*`, `internal.*`, or any other reserved namespace.
- `launch_context` is merged during `INITIALIZE`, after graph attributes are mirrored into `graph.*` and before the initial checkpoint is written.

This mechanism is for first-class run state, not prompt templating. The built-in prompt variable expansion rules remain unchanged (see Section 4.5 and Section 9.2).
...
FUNCTION run(graph, config):
    ...
    apply_launch_context(config.launch_context, context)
...
When `codergen.response_contract="status_envelope"`, the runtime appends a shared structured-output appendix to the prompt that enumerates the allowed top-level Outcome keys and forbids extra ones. That appendix also includes node-specific `context_updates` guidance derived from the resolved normalized `spark.writes_context` contract: nodes with declared writes are shown the exact allowed keys and flat dotted-key rules, while nodes without declared writes are told not to emit `context_updates`. Use this on codergen nodes that must return a structured status envelope; do not set it on non-codergen nodes.

For response-contract codergen nodes, malformed contract output is not treated as a modeled business `fail`. The runtime performs up to `codergen.contract_repair_attempts` bounded same-thread repair turns, feeding the exact contract validation error back to the same agent thread and asking it to re-emit only corrected final output for the same decision. Those repair turns reuse the same node-specific `context_updates` contract text derived from `spark.writes_context`, so empty or missing write declarations are repaired toward no `context_updates` and non-empty declarations are repaired toward only the listed keys. The same repair loop also applies when structured `context_updates` keys violate the node's declared `spark.writes_context` allowlist after runtime normalization. `spark.writes_context` is the exact runtime write allowlist for node-authored `context_updates`; if a node omits that declaration, node-authored writes are not allowed. Runtime enforcement remains the source of truth if the model still violates the contract. The companion `spark.reads_context` metadata still does not restrict generic runtime reads, but Codergen now uses it as the deterministic prompt-projection contract for declared live-context inputs and renders absent declared keys as `<missing>`. This repair budget is distinct from node `max_retries`, which still controls full node reruns for runtime failures.
...
| Prefix        | Purpose                                        |
|---------------|------------------------------------------------|
| `context.*`   | Semantic state shared between nodes. May be seeded by the host at launch and extended during execution via `context_updates`. |
