...
Conversation turns and segments continue to persist plain string content. Markdown interpretation, when used, is a frontend-only rendering rule applied to assistant replies, expanded thinking, and existing plan/result cards through the shared Home/Run renderer. That presentation boundary does not introduce a workspace HTTP, API, schema, or persisted-state change.
